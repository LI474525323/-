...
![image-20230106191130148](C:\Users\Caly\AppData\Roaming\Typora\typora-user-images\image-20230106191130148.png)

- ctrl+alt+b从接口跳转到实现类，从接口名进入
...
> 登录凭证的sessionid就在cookie中

拦截器代码，utils里面的LoginInterceptor这个类
...
请求接口，UserController里面（应该是点击我的时候会请求此接口）
...
```java
    ...
    public Result sendCode(String phone, HttpSession session) {
        ...
        // 3. 符合生成验证码
        String code = RandomUtil.randomNumbers(6);
        // 4. 保存验证码到redis,设置有效期，防止用户一直点击造成内存占用过多
...
public Result login(LoginFormDTO loginForm, HttpSession session) {
    ...
    // RedisTemplate<String, String>要求key,value都是string,而userMap里面的id是long类型，因此
    // 会出现类型转换错误，使用CopyOptions.create()进行修改
...
```java
    ...
    // 并不是springboot里面的，所以只能使用构造器进行赋值,在配置拦截器的地方进行注入
    ...
    @Override
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        // 1.获取请求头的token,authorization前端发过来的
...
public class MvcConfig implements WebMvcConfigurer {
    @Resource
    public StringRedisTemplate stringRedisTemplate; // 在这里进行注入
```

![image-20230212192212127](C:\Users\Caly\AppData\Roaming\Typora\typora-user-images\image-20230212192212127.png)
...
**ShopController里面queryShopById里面调用的方法**
...
public class ShopServiceImpl extends ServiceImpl<ShopMapper, Shop> implements IShopService {
    ...
    public Result queryById(Long id) {
        ...
        String cacheJson = stringRedisTemplate.opsForValue().get(key); // 对象在str是json格式
        //2.判断缓存中是否命中
        if (StrUtil.isNotBlank(cacheJson)) {
            // 3.命中直接返回商品信息
            Shop shop = JSONUtil.toBean(cacheJson, Shop.class);
            return Result.ok(shop);
        }
        // 4.未命中则根据id查询数据库,mybatis里面提供的方法
...
![image-20230213145542772](C:\Users\Caly\AppData\Roaming\Typora\typora-user-images\image-20230213145542772.png)
...
- ShopController里面updateShop调用的方法
- Transactional注解是添加事物，更新数据库和删除缓存需要一致发生，原子性
...
- **ShopServiceImpl**的queryById方法改进
...
public Result seckillVoucher(Long voucherId) {
    ...
    // 6.1.订单id
    // 不能使用new,应该是注入，这个看一下为什么，应该是没有放到bean里面
...
* 修改业务代码**VoucherOrderServiceImpl**
...
```java
// required = false代表可以有也可以没有这个请求参数
@GetMapping("/of/type")
